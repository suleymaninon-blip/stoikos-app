...
- `scripts/generate-audio.ts` — ElevenLabs ses üretimi. Sesler: tr=Sukru Terzi, en=Donovan, de=David, ru=Artem Lebedev, fr=Yann, es=Miguel. `npm run gen-audio` (yalnız kavram; `--all` ile hepsi). Mevcut dosyaları atlar — yeniden üretmek için önce ilgili mp3'leri silmek gerekir.
- ⚠️ **Ses boyutu tavana yakın** (ölçüm: 7 Eylül 2026). `assets/audio` = **36 MB**; bunun 14 MB'ı 72 kavram mp3'ü, 16 MB'ı 144 alıntı mp3'ü. Kavram sesi bilerek yalnız **ilk paragrafı** okuyor (~120 karakter, ~10 sn). Tüm `desc`'i okusaydı metin 8.885 → 72.372 karaktere (**8,1 kat**) çıkar, kavram sesi ~113 MB olur ve klasör **~135 MB**'a fırlardı. Uygulamaya gömülü ses bu ölçekte sürdürülemez: App Store hücresel indirme sınırına dayanır ve indirme dönüşümünü düşürür.
  👉 Sonuç: "kavramların sesli anlatımı"nı zenginleştirme kararı (Plus özelliği, madde 7) **önce sesi talep üzerine indirilir hâle getirmeyi** gerektirir. Gömülü modelde metni uzatma.
...
- `app/programs.tsx` + `constants/programs.ts` — **Programlar**: rehberli çok günlük yolculuklar. 5 program × 7 gün = 35 gün (Kontrol Dairesi, İç Sakinlik, **Öfkeyle Çalışmak**, **Başkalarının Gözü**, **Zamanın Kısalığı** — son üçü Eylül 2026'da eklendi), altı dilde tam (fr/es Eylül 2026'da eklendi).
  - İlk ikisi farklı türde: *Kontrol Dairesi* temeli tek bir ayrım üzerinden kuruyor, *İç Sakinlik* klasik egzersizlerin turu. Son üçü ise tek bir yöntemi yedi güne yayıyor.
  - Öfke yayı: erteleme → istemsiz olanı ayır → altındaki yargıyı bul → yargıyı değiştir → bedeli gör → beklentiyi ayarla → alışkanlığa çevir.
  - Başkalarının Gözü yayı: say → seyirciyi adlandır → seyirciyi küçült → kıyasın hilesini gör → kendi ölçünü koy → onaysızlığı dene → övgüyü de aynı kefeye koy.
  - Zamanın Kısalığı yayı: ölç → adlandır → başla → tarih ver → koru → günü kapat → önceliği seç. Kaynak damarı Seneca'nın *De Brevitate Vitae*'si — diğer dört programda kullanılmamıştı.
  - **Yeni program eklerken:** `PROGRAMS_RAW`'a bir nesne eklemek yeterli; `app/programs.tsx` sayı varsayımı tutmuyor. Simge ve renk her programda ayrı olmalı (şu an ◎ yeşil, ✦ altın, ▲ kor, ◐ arduvaz, ⧗ erik). CLAUDE.md, `docs/urun-degerlendirmesi.md` (envanter tablosu + elde tutma bulgusu) ve broşür de güncellenmeli. İlerleme cihazda (`stoikos_program_<id>`), her gün bir öncekini tamamlayınca açılıyor. Ana ekrandan erişiliyor. **Elde tutmanın en ucuz kaldıracı burası** — üretim maliyeti yalnız metin, ve koç aboneliğinin aksine ChatGPT ikame edemez.
- `app/challenge*.tsx`.
...
7. 💳 **Para modeli: STOIKOS PLUS** (6 Eylül 2026'da genişletildi)
   ...
   - **Plus:** koç, tüm programlar, kavramların sesli anlatımı, sonradan eklenen içerik. **$6,99/ay veya $49,99/yıl** (%40 tasarruf, yıllık önseçili), **14 gün ücretsiz deneme.**
     ⚠️ Sesli anlatımı uzatmak isteyince önce yukarıdaki **ses boyutu** notunu oku (Mimari bölümü): gömülü modelde metni uzatmak klasörü ~135 MB'a çıkarıyor.
