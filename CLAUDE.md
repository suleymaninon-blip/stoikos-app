...
- `app/programs.tsx` + `constants/programs.ts` — **Programlar**: rehberli çok günlük yolculuklar. 3 program × 7 gün = 21 gün (Kontrol Dairesi, İç Sakinlik, **Öfkeyle Çalışmak** — üçüncüsü Eylül 2026'da eklendi), altı dilde tam (fr/es Eylül 2026'da eklendi). Öfke programının yayı: erteleme → istemsiz olanı ayır → altındaki yargıyı bul → yargıyı değiştir → bedeli gör → beklentiyi ayarla → alışkanlığa çevir. İlerleme cihazda (`stoikos_program_<id>`), her gün bir öncekini tamamlayınca açılıyor. Ana ekrandan erişiliyor. **Elde tutmanın en ucuz kaldıracı burası** — üretim maliyeti yalnız metin, ve koç aboneliğinin aksine ChatGPT ikame edemez.
- `app/challenge*.tsx`.
