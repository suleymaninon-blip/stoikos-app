...
- `app/programs.tsx` + `constants/programs.ts` — **Programlar**: rehberli çok günlük yolculuklar. 4 program × 7 gün = 28 gün (Kontrol Dairesi, İç Sakinlik, **Öfkeyle Çalışmak**, **Başkalarının Gözü** — son ikisi Eylül 2026'da eklendi), altı dilde tam (fr/es Eylül 2026'da eklendi).
  - İlk ikisi farklı türde: *Kontrol Dairesi* temeli tek bir ayrım üzerinden kuruyor, *İç Sakinlik* klasik egzersizlerin turu. Son ikisi ise tek bir yöntemi yedi güne yayıyor.
  ...
  - Başkalarının Gözü yayı: say → seyirciyi adlandır → seyirciyi küçült → kıyasın hilesini gör → kendi ölçünü koy → onaysızlığı dene → övgüyü de aynı kefeye koy. İlerleme cihazda (`stoikos_program_<id>`), her gün bir öncekini tamamlayınca açılıyor. Ana ekrandan erişiliyor. **Elde tutmanın en ucuz kaldıracı burası** — üretim maliyeti yalnız metin, ve koç aboneliğinin aksine ChatGPT ikame edemez.
- `app/challenge*.tsx`.
